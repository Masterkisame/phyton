...
pygame.display.set_caption("Puzzle de Gemas con Sprites y Animación")
...
# Posiciones animadas en píxeles
positions = {(r,c):(c*(SIZE+MARGIN)+MARGIN, r*(SIZE+MARGIN)+MARGIN) for r in range(ROWS) for c in range(COLS)}

def draw_board():
    for r in range(ROWS):
        for c in range(COLS):
            name = board[r][c]
            x_target = c*(SIZE+MARGIN)+MARGIN
            y_target = r*(SIZE+MARGIN)+MARGIN
            x,y = positions[(r,c)]
            # interpolar hacia destino
            if y < y_target:
                y += 10  # velocidad de caída
                if y > y_target: y = y_target
            positions[(r,c)] = (x_target,y)
            screen.blit(GEMAS[name], (x,y))
            ...
            if board[r][c] == "Roca":
                for k in range(r-1, -1, -1):
                    if board[k][c] != "Roca":
                        board[r][c] = board[k][c]
                        board[k][c] = "Roca"
                        # animar caída desde posición anterior
                        positions[(r,c)] = positions[(k,c)]
                        break
...
def reset_game():
    global board, selected, score, gem_count, game_over, positions
    board = new_board()
    selected = []
    score = 0
    gem_count = {name:0 for name in GEMAS.keys()}
